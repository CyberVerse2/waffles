    ...
    <div style={{ background: "#0F0F10", border: "1px solid rgba(255,255,255,.06)", borderRadius: 16, padding: 14, boxShadow: "inset 0 1px 0 rgba(255,255,255,0.04)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {missions.map((m, i) => {
          ...
          const isAction = Boolean(m.onClick);
          const accent = m.icon === "verify" ? "#00CFF2" : m.icon === "setup" ? "#FFC931" : null;
          ...
          const iconColor = done ? "#00CFF2" : accent ?? "rgba(255,255,255,.4)";
          const iconBg = done
            ? "rgba(0,207,242,.15)"
            : accent
              ? `${accent}22`
          ...
          const innerContent = (
            <>
              <div style={{ width: 30, height: 30, borderRadius: 8, background: iconBg, border: `1px solid ${iconBorder}`, display: "flex", alignItems: "center", justifyContent: "center", color: iconColor, flexShrink: 0 }}>
                {done ? (
                  ...
                  <svg width="14" height="14" viewBox="0 0 24 24"><path d="M12 3l8 4v5c0 5-3.5 8-8 9-4.5-1-8-4-8-9V7l8-4z" stroke="currentColor" strokeWidth="2" fill="none" strokeLinejoin="round" /><path d="M9 12l2 2 4-4" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" /></svg>
                ) : m.icon === "xp" ? (
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4, gap: 8 }}>
                  <span style={{ fontSize: 12, fontWeight: 800, color: "#fff", display: "inline-flex", alignItems: "center", gap: 6, minWidth: 0 }}>
                    <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.label}</span>
                    {m.badge && (
                      <span style={{ fontSize: 8, fontFamily: "Archivo Black", letterSpacing: 0.8, padding: "2px 5px", borderRadius: 4, background: accent ? `${accent}26` : "rgba(255,255,255,.08)", color: accent ?? "rgba(255,255,255,.6)", flexShrink: 0 }}>{m.badge}</span>
                    )}
                  </span>
                ...
                <div style={{ height: 5, borderRadius: 99, background: "rgba(255,255,255,.05)", overflow: "hidden" }}>
                  <div style={{ width: `${pct}%`, height: "100%", background: done ? "linear-gradient(90deg,#00CFF2,#5DDDF0)" : accent ? `linear-gradient(90deg, ${accent}, ${accent}99)` : "linear-gradient(90deg, #FFC931, #F5BB1B)", transition: "width .4s" }} />
                </div>
              </div>
              <div style={{ fontSize: 10, fontWeight: 800, color: done ? "#00CFF2" : accent ?? "rgba(255,255,255,.55)", letterSpacing: 0.4, minWidth: 40, textAlign: "right" }}>{m.reward}</div>
            </>
          );
          if (isAction) {
            return (
              <button key={i} type="button" className="pressable" onClick={m.onClick} style={rowStyle} aria-label={`${m.label}, reward ${m.reward}`}>
                {innerContent}
              </button>
            );
          }
